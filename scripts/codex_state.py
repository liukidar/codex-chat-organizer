...
def codex_home_from_arg(value: str | None) -> Path:
    ...
    env_value = Path.home()
    if "CODEX_HOME" in __import__("os").environ:
        return Path(__import__("os").environ["CODEX_HOME"]).expanduser()
...
def backup_state(codex_home: Path, db_path: Path, reason: str) -> str:
    ...
    for candidate in (db_path, Path(str(db_path) + "-wal"), Path(str(db_path) + "-shm"), codex_home / "session_index.jsonl"):
        if candidate.exists():
...
def snippet(text: str, query: str) -> str:
    ...
    end = min(len(compact), index + len(query) + 130)
...
def list_command(args: argparse.Namespace) -> dict[str, Any]:
    ...
    schema = schema_payload(codex_home)
    if not schema["ok"]:
        return {"ok": False, "schema": schema, "threads": []}
    ...
    return {"ok": True, "schema": schema, "threads": threads, "query": args.query or ""}
...
def archive_command(args: argparse.Namespace) -> dict[str, Any]:
    codex_home = codex_home_from_arg(args.codex_home)
    db_path = assert_schema(codex_home)
    backup_dir = backup_state(codex_home, db_path, "archive" if args.archived else "unarchive")
    ...
    with connect(db_path, readonly=False) as con:
        ...
        con.execute(
            ...
            where id = ?
            """,
            (archived, archived_at, now_s, now_ms, args.thread_id),
        )
        con.commit()
    return {"ok": True, "backup_dir": backup_dir, "archived": bool(archived)}
